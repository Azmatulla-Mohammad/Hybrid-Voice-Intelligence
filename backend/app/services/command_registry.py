    ...
    def resolve(self, text: str) -> Optional[dict]:
        ...
        for rule in self._rules:
            if rule.matcher(normalized):
                return rule.handler(normalized)
        return None
...
REGISTRY.register(
    CommandRule(
        name="news",
        matcher=lambda t: "news" in t or "headlines" in t,
        handler=lambda t: _action(
            SystemSkill.get_news_headlines(
                t.replace("news", "").replace("headlines", "").strip() or "latest"
            )
        ),
    )
)
...
def _handle_open(text: str) -> str:
    ...
    if "docs" in target or "documentation" in target:
        return SystemSkill.search_google(f"{target} documentation")
